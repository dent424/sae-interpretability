...
```bash
py -3.12 -m modal run src/modal_interpreter.py::analyze_feature_json --feature-idx $ARGUMENTS
```
...
py -3.12 -m modal run src/modal_interpreter.py::batch_test -- \
    --feature-idx $ARGUMENTS \
    --texts "positive 1|positive 2|...|negative 1|negative 2|..."
```
...
py -3.12 -m modal run src/modal_interpreter.py::analyze_feature_json -- \
    --feature-idx $ARGUMENTS --run-ablation
```
